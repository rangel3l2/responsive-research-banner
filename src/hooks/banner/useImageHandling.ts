...
export const useImageHandling = (
  ...
  const handleImageInsert = () => {
    ...
    const imageIndex = formData.images

The error occurs because the `useImageHandling` hook is trying to access `formData` without it being passed as a parameter. Let's fix this by properly passing the formData and setFormData as parameters to the hook.

<lov-code>
Let's update the `useImageHandling.ts` file to properly handle the formData:

<lov-write file_path="src/hooks/banner/useImageHandling.ts">
import { BannerFormData } from '@/models/formData';
import { MAX_IMAGE_SIZE_KB } from '@/utils/docxStyles';
